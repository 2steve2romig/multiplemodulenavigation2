...
| 8 | ~~21 CFR Part 11 §11.10 compliance audit — immutable audit trail~~ — **Phase 1 + 2a resolved 2026-06-24.** Phase 1: Migration 003 (audit_log + electronic_signatures tables, RLS), Migration 004 (immutability trigger, org/org_entitlement RLS hardening). Phase 2a: Migration 005 — four PostgreSQL RPC functions wrapping each regulated mutation + audit_log INSERT in a single transaction (fail-closed). All four mutation routes updated to call `supabase.rpc()`. `writeAuditLog()` retained for non-regulated paths. Integration audit-atomicity tests added. Remaining: Phase 2b (e-signatures per IQ module) + Phase 3 (validation protocol). | Engineering + SME | Phase 1+2a closed — Phase 2b+3 pending |
...
| 14 | ~~Migration — FK from `entitlements.module_id` → `module_manifests.id` and `org_entitlements.module_id` → `module_manifests.id` (`ON DELETE RESTRICT`)~~ — **Resolved 2026-06-24.** Migration `006_restrict_and_softdelete.sql` adds both FKs as `NOT VALID` (skips existing-row validation). Run `ALTER TABLE entitlements VALIDATE CONSTRAINT entitlements_module_id_fkey` after backfilling `module_manifests` for all existing entitlement module_ids. | Engineering | Closed |
| 15 | ~~`PATCH /api/entitlements/:id` — missing mutation route~~ — **Resolved 2026-06-24.** `api/entitlements/[id].js` implemented using `update_entitlement_with_audit` RPC (migration 006). Captures `before_state`/`after_state` for ALCOA. Integration tests added in `test/integration/admin-routes.test.js`. | Engineering | Closed |
| 16 | ~~Soft-delete (`deleted_at`) on `tenants` and `organizations` + `ON DELETE RESTRICT` on audit_log FKs~~ — **Resolved 2026-06-24.** Migration 006: `audit_log.tenant_id` and `audit_log.org_id` FKs changed from `ON DELETE SET NULL` → `ON DELETE RESTRICT`. `deleted_at timestamptz` added to `tenants` and `organizations`. Schema constraint integration test added in `test/integration/isolation.test.js`. | Engineering | Closed |
| 19 | Local dev orchestration — `Makefile` with `make dev` (runs `supabase start` then `vercel dev`); `npm run dev` alias for `vercel dev`. Implemented 2026-06-24. See `Makefile` and `package.json`. | Engineering | Closed |
| 20 | ~~`updated_at` auto-trigger function~~ — **Resolved 2026-06-25.** Migration `007_users_auth_columns.sql` creates `set_updated_at()` BEFORE UPDATE trigger function and attaches it to all 6 mutable tables: `tenants`, `users`, `organizations`, `entitlements`, `org_entitlements`, `module_manifests`. Immutable tables (`audit_log`, `electronic_signatures`) excluded intentionally. | Engineering | Closed |
| 21 | ~~Module manifests backfill + FK validation~~ — **Resolved 2026-06-25.** Migration `008_validate_module_fks.sql` seeds all 14 modules (`ON CONFLICT DO NOTHING`) then runs `VALIDATE CONSTRAINT` on `entitlements_module_id_fkey` and `org_entitlements_module_id_fkey`. Ghost entitlement prevention now fully enforced for all rows. Integration test added: POST with non-existent `module_id` returns 500. | Engineering | Closed |
| 17 | ~~`external_auth_id text UNIQUE` + `auth_provider text` columns on `users` table~~ — **Resolved 2026-06-25.** Migration `007_users_auth_columns.sql` adds both columns. `external_auth_id` has a UNIQUE constraint and partial index (`WHERE external_auth_id IS NOT NULL`). `auth_provider` is nullable until auth is wired per ADR-004. Integration test added: duplicate `external_auth_id` insert is rejected. | Engineering | Closed |
